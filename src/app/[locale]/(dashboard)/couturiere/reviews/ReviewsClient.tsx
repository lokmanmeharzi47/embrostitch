...
export default function ReviewsClient({ initialReviews }: { initialReviews: ReviewData[] }) {
  const [filterRating, setFilterRating] = useState("all");

  const filtered =
    filterRating === "all"
      ? initialReviews
      : initialReviews.filter((r) => r.rating === parseInt(filterRating));
  ...
  const ratingBreakdown = [5, 4, 3, 2, 1].map((star) => ({
    ...
    pct:
      initialReviews.length > 0
        ? (initialReviews.filter((r) => r.rating === star).length / initialReviews.length) * 100
        : 0,
  }));
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-10">
        <div className="bg-card border border-border rounded-xl p-6 shadow-sm flex flex-col items-center justify-center text-center">
          ...
          <p className="text-xs text-muted-foreground">
            Basé sur {initialReviews.length} avis
          </p>
        </div>
